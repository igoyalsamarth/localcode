"""
Persist LLM token usage for the GitHub coder workflow (pricing / planning).
"""
...
def _resolve_repo(
    session, issue: IssueOpenedForCoder
) -> tuple[UUID | None, UUID | None]:
    stmt = select(Repository).where(
        Repository.owner == issue.owner,
        Repository.name == issue.repo_name,
    )
...
def record_coder_workflow_usage(
    ...
    *,
    ...
    """
    Insert one ``CoderWorkflowUsage`` row after an agent run.

    Does not raise — logs on failure so billing never breaks the coder.
    """
    ...
    try:
        with session_scope() as session:
            ...
            session.add(row)
        logger.info(
            "Recorded coder workflow usage: %s#%s model=%s in=%s out=%s total=%s",
            issue.full_name,
            issue.issue_number,
            model_label,
            inp,
        ...
        logger.exception(
            "Failed to record coder workflow usage for %s#%s",
            issue.full_name,
            issue.issue_number,
        )
